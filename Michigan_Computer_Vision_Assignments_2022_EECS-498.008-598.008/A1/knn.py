...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    train_x = x_train.clone().view(num_train, -1)
    test_x = x_test.clone().view(num_test, -1)
    for i  in range(train_x.shape[0]):
      for j in range(test_x.shape[0]):
        dists[i, j] = (train_x[i] - test_x[j]) @ (train_x[i] - test_x[j]).T
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    test_x = x_test.clone().view(num_test, -1)
    for i in range(train_x.shape[0]):
      dists[i] = torch.diagonal( (test_x - train_x[i]) @ (test_x - train_x[i]).T , 0)
      #dists[i] = torch.diagonal( (test_x - train_x[i]).mm( (test_x - train_x[i]).T ) , 0)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #print(new_train_x[:, 5, :] == train_x[5, :]) #control
    #print(test_x.shape)
    #print(train_x.shape)
    #print(new_train_x.shape)
    #print((new_train_x - test_x).shape)
    #print(new_train_x[0])
    #print(torch.permute(new_train_x, (1, 0, 2))[0])
    ##########################################################################
    train_x = x_train.clone().view(num_train, -1)
    test_x = x_test.clone().view(num_test, -1)
    new_train_x = torch.broadcast_to( train_x, ( num_test, train_x.shape[0], train_x.shape[1]) )
    new_train_x = torch.permute(new_train_x, (1, 0, 2))
    dists = torch.sum( (new_train_x - test_x) ** 2 , dim= 2, keepdim= False)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    top_k = torch.topk(dists, dim=0, largest=False, k= k).indices
    for each_test_i in range(y_pred.shape[0]):
      votes = y_train[ top_k[:, each_test_i] ]
      y_pred[each_test_i] = torch.mode(votes).values
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred

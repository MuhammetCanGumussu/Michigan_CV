...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        y = x.view(x.shape[0], -1) # reshape -> (N,D) 
        out = torch.addmm(b, y, w)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        Inputs:
        - dout: Upstream derivative, of shape (N, M)   #çıkan tensor shape'inde olmalı upstream gradient
        - cache: Tuple of:
        ...
        ##################################################
        y = x.view(x.shape[0], -1) # reshape -> (N,D) 
        dx = torch.reshape(torch.mm(dout, w.T), x.shape)    #x shape : (N, d_1, ... d_k), dolayısı ile dx de öyle olmalı
        dw = torch.mm(y.T, dout)
        db = torch.sum(dout, dim=0) # çıktının b ye göre türevi M boyutlu 1'ler vektoru. Bunun dout ile çarpımı (klasik matris, vektor carpımı) : dout'un row'ları boyunca toplanmasına eşittir, deneyebilirsin 
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class TwoLayerNet(object):
    ...
    def load(self, path, dtype, device):
        ...
        for p in self.params:
            self.params[p] = self.params[p].type(dtype).to(device)  #yüklenen parametrelerin type'leri, device'ları ayarlanıyor
    ...
    def loss(self, X, y=None):
        ...
        # scores variable.                                          #
        #############################################################
        a1, linear_relu_cache = Linear_ReLU.forward(X, self.params["W1"], self.params["b1"])
        scores, linear2_cache = Linear.forward(a1, self.params["W2"], self.params["b2"])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dh, dw2, db2 = Linear.backward(dout, linear2_cache)
        grads["W2"] = dw2 + 2 * self.reg * self.params["W2"]
        grads["b2"] = db2
        _, dw1, db1 = Linear_ReLU.backward(dh, linear_relu_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        - seed: If not None, then pass this random seed to the dropout
          ...
          can gradient check the model. # seed sayesinde random sample'lar deterministic dolayısı ile dropout ve model de
        - dtype: A torch data type object; all computations will be
          performed using this datatype. float is faster but less accurate,
          so you should use double for numeric gradient checking.
        - device: device to use for computation. 'cpu' or 'cuda'
        ...
        #######################################################################  
        #first layer
        self.params["W{}".format(1)] = torch.randn(input_dim, hidden_dims[0], dtype= self.dtype, device= self.device) * weight_scale
        self.params["b{}".format(1)] = torch.zeros(hidden_dims[0], dtype= self.dtype, device= self.device)
        
        for l+2 in range(self.num_layers):
          self.params["W{}".format(l)] = torch.randn(hidden_dims[l-1], hidden_dims[l], dtype= self.dtype, device= self.device) * weight_scale
          self.params["b{}".format(l)] = torch.zeros(hidden_dims[l], dtype= self.dtype, device= self.device)
        
        #last layer  
        self.params["W{}".format(self.num_layers)] = torch.randn(hidden_dims[self.num_layers], num_classes, dtype=self.dtype, device=self.device) * weight_scale 
...
def create_solver_instance(data_dict, dtype, device):
    ...
    #############################################################
    #başlangıç modeli yaklaşık %36 acc'ye sahip, lr ile oynayacağız
    solver = Solver(model, data_dict, lr_decay=0.9, optim_config={'learning_rate':1e-1}, num_epochs=10)
    solver.train()
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################

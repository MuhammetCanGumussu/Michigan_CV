...
def _extract_tensors(dset, num=None, x_dtype=torch.float32):
    ...
    x = torch.tensor(dset.data,
                     dtype=x_dtype).permute(0, 3, 1, 2).div_(255) #CIFAR10 'da data set dim orderı -> Batch,H,W,C şeklinde, bunu değiştirdik
...
def cifar10(num_train=None, num_test=None, x_dtype=torch.float32):
    ...
    - y_train: int64 tensor of shape (num_train, 3, 32, 32)  #? yazım hatası -> (N,) diğer y_test de böyle olmalı
...
def preprocess_cifar10(
    ...
    - validation_ratio: Float in the range (0, 1) giving the
      fraction of the train set to reserve for validation      #eğitim verimiz var 

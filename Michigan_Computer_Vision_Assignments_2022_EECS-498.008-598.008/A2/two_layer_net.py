...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    # shape (N, C).                                                            #
    ############################################################################
    lineer_H1 = torch.addmm(b1, X, W1)  # returns -> (N,H)
    hidden = torch.clamp(lineer_H1, min=0)  # relu nonlinearity
    scores = torch.addmm(b2, hidden, W2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    ############################################################################
    scores = (scores.T - torch.max(scores, dim=1)[0]).T   #numeric stability
    scores_exp = torch.exp(scores)
    sum_of_exp = torch.sum(scores_exp, dim=1)   
    data_loss = torch.sum ( -1 * torch.log( scores_exp[torch.arange(N), y] / sum_of_exp ) ) / N
    ...
    # tensor of same size                                                     #
    ###########################################################################
    factor = torch.div(scores_exp, sum_of_exp.view(-1, 1))
    factor[torch.arange(N), y] = -1 * (sum_of_exp - scores_exp[torch.arange(N), y]) / sum_of_exp 
    factor = factor.div(N) 
    
    grads["b2"] = torch.sum(factor, dim=0) 
    grads["W2"] = h1.t().mm(factor) + 2 * reg * W2 
...
def nn_get_search_params():
    ...
    # classifier.                                                             #
    ###########################################################################
    learning_rates = [1e-1,  3e-1,  5e-1,  7e-1,  9e-1, 1e0, 1.5]
    hidden_sizes = [400, 450, 500, 550]
    regularization_strengths = [5e-5, 7e-5, 9e-5, 1e-4,  3e-4,  5e-4,  7e-4]
    learning_rate_decays = [0.95, 0.90, 0.80, 0.85]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    best_reg = 0
    best_lr_dec = 0
    param_tuple =  nn_get_search_params()

    learning_rate = param_tuple[0]
    hidden_units = param_tuple[1]
    regularization = param_tuple[2]
    learning_rate_decay = param_tuple[3]

    for hidden in hidden_units:
      for lr in learning_rate:
        for reg in regularization:
          for lr_dec in learning_rate_decay:
            model = TwoLayerNet(3 * 32 * 32, hidden, 10, device=data_dict['X_train'].device)
            ...
            if val_acc > best_val:
              ...
              best_lr_dec = lr_dec
            # Print out results.
            print('lr= %e, reg= %e, hidden= %e, lr_dec= %e, val accuracy= %f' % (
                       lr, reg, hidden, lr_dec, val_acc))
    print("best h:%e, lr:%e, r:%e, lr_dec:%e" %(best_h, best_l, best_r, best_lr_dec))  
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
